...
int amds_ras_read(amds_gpu_t *gpu) {
    ...
    if (g_amds_logger && gpu->ras.available && (gpu->ras.corrected > 0 || gpu->ras.uncorrected > 0 || gpu->ras.bad_pages > 0)) {
        amds_log_printf(g_amds_logger, "[RAS] GPU%d: CE=%lu UE=%lu BAD=%lu",
